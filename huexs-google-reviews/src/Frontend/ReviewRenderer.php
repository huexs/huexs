...
class ReviewRenderer {
	...
	public function __construct(
		...
		private bool $brandingRequired = false
	) {}

	/**
	 * @param object[]   $reviews
	 * @param array      $args    Atributos normalizados del shortcode.
	 * @param array|null $summary Datos del resumen o null.
	 */
	...
	public function attributionHtml(): string {
		$html = '<div class="hgr-attribution">';
		if ( ! empty( $this->settings['show_google_logo'] ) ) {
			$html .= $this->googleLogoHtml();
		}
		$html .= '<span>' . esc_html__( 'Reseñas de Google', 'huexs-google-reviews' ) . '</span>';
		if ( $this->brandingRequired ) {
			$html .= '<span class="hgr-attribution__by">'
				. esc_html__( '· por Huexs', 'huexs-google-reviews' )
				. '</span>';
		}
		$html .= '</div>';
		return $html;
	...
	public function googleLogoHtml(): string {
		return '<svg class="hgr-google-g" viewBox="0 0 48 48" aria-hidden="true" focusable="false">'
	...
	/** Clases del contenedor: tema + estilo de tarjeta. */
	public function wrapClasses( string $layout ): string {
		$theme = (string) ( $this->settings['theme'] ?? 'light' );
		if ( ! in_array( $theme, array( 'light', 'dark', 'transparent' ), true ) ) {
			$theme = 'light';
		}
		$card = (string) ( $this->settings['card_style'] ?? 'shadow' );
		if ( ! in_array( $card, array( 'shadow', 'border', 'flat' ), true ) ) {
			$card = 'shadow';
		}
		return implode(
			' ',
